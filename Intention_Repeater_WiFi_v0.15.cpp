/*
Intention Repeater WiFi v0.15
Created by Anthro Teacher, WebGPT and Claude 3 Opus.
Help received from nathanmyersc.
...
int main()
{
    ...
    cout << "Intention Repeater WiFi Broadcaster v0.15" << endl;
    ...
    cout << "Remove Spaces (y/N): ";
    getline(cin, remove_spaces);
    transform(remove_spaces.begin(), remove_spaces.end(), remove_spaces.begin(), ::tolower);

    if (remove_spaces != "y" && remove_spaces != "yes")
    {
        remove_spaces = "n";
    }
    else
    ...
    if (multithreaded == "y")
    {
        for (int i = 0; i < NUM_THREADS + 1; ++i)
        {
            ...
            broadcastAddr[i].sin_port = htons(11111 + i);
        }
        for (int i = 0; i < NUM_THREADS; ++i)
        {
    ...
    else // Not Multithreaded
    {
        ...
        broadcastAddr[0].sin_addr.s_addr = inet_addr("255.255.255.255");
        broadcastAddr[0].sin_port = htons(11111);
        while (running)
        {
            broadcastMessage(threadSocks[0], intention_multiplied, broadcastAddr[0]);
            ++freq;
