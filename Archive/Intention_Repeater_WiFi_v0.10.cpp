...
std::pair<std::string, int> multiplyIntention(const std::string& intention, const std::string& boost) {
    std::string intention_multiplied = "", intention_boosted = intention + "|0010110|";
    int multiplier = 0;

    // Continue appending until the length exceeds BUFFER_SIZE
    if (boost=="y") {
        while (intention_multiplied.length() + intention_boosted.length() <= BUFFER_SIZE) {
            intention_multiplied += intention_boosted;
...
int main()
{
    ...
    cout << "Intention Repeater WiFi Broadcaster v0.10" << endl;
    ...
    string boost;
    cout << "Boost [Adds 0010110] (y/N): ";
    getline(cin, boost);
    transform(boost.begin(), boost.end(), boost.begin(), ::tolower);

    auto result = multiplyIntention(intention, boost);

    intention_multiplied = result.first;
    multiplier = result.second;

    if (boost != "y" && boost != "yes")
    {
        boost = "n";
    }
