...
void broadcastThread(int threadIndex)
{
    ...
    while (running)
    {
        broadcastMessage(sock, intention, broadcastAddr);
        ++iterations;
        ++freq;
    }
}
...
int main()
{
    ...
    cout << "Intention Repeater WiFi Broadcaster v0.6" << endl;
    ...
    while (running)
    {
        if (multithreaded != "y")
        {
            broadcastMessage(threadSocks[0], intention, broadcastAddr);
            ++threadIterations[0];
            ++threadFreq[0];
        }
        ...
        if (chrono::duration_cast<chrono::milliseconds>(now - lastUpdate).count() >= 1000)
        {
            ...
            cout << "\rBroadcasting: [" << formatTime(seconds) << "] " << formatNumber(totalIterations) << " Repetitions (" << formatFreq(totalFreq) << ")" << string(5, ' ') << "\r";
            cout.flush();
            lastUpdate = now;
            ++seconds;
        }
