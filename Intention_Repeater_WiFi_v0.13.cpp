...
void broadcastMessage(const SOCKET &sock, const std::string &message, struct sockaddr_in &broadcastAddr)
{
    ...
    while (bytesSent < bufferSize)
    {
        ...
        if (result == SOCKET_ERROR)
        {
            ...
            {
                std::cerr << "\rBroadcasting: Waiting..." << string(40, ' ') << flush;
...
int main()
{
    ...
    cout << "Intention Repeater WiFi Broadcaster v0.13" << endl;
    ...
    if (multithreaded == "y")
    {
        ...
        while (running)
        {
            ...
            if (chrono::duration_cast<chrono::milliseconds>(now - lastUpdate)
              .count() >= 1000) {
              for (int i = 0; i < NUM_THREADS; ++i) {
                //iterations += threadIterations[i].load();
